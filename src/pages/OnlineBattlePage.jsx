...
function getBattleAbilities(character, state) {
  if (!character) return []
  if (isTokata(character) && state?.tokata_transformation) return Array.isArray(state.tokata_transformation.copied_abilities) ? state.tokata_transformation.copied_abilities : []
...
function OnlineBattleCharacterCard({ character, participant, state, isActive, isMine, side }) {
  const displayCharacter = getBattleCharacterForDisplay(character, state)
  ...
  const image = getImage(displayCharacter)
  return (
    <article className={`battle-character battle-character-${side} ${isActive ? 'is-active' : ''} ${state?.defending ? 'is-defending' : ''} ${hp <= 0 ? 'is-defeated' : ''}`} data-user-id={participant.user_id} data-character-id={state?.character_id || ''} data-ultimate-image={character?.profile?.ultimateImage || ''} data-ultimate-name={character?.profile?.ultimateName || 'Técnica definitiva'}>
      <div className="battle-character-heading"><div><p className="battle-side-label">{isMine ? 'VOS' : 'OPONENTE'}</p><h2>{displayCharacter?.name || 'Personaje'}</h2></div><strong className="battle-hp-value">{Math.ceil(hp)}</strong></div>
      ...
      <div className={`battle-energy ${energy >= 100 ? 'energy-is-full' : ''}`}><div className="battle-energy-header"><span>ENERGÍA</span><span>{Math.round(energy)}%</span></div><div className="battle-energy-bar"><div className="battle-energy-fill" style={{ width: `${energyPercentage}%` }} /></div></div>
      {states.length > 0 && <div className="battle-status-list">{getBattleStateInfo(states).map((battleState) => <span className={`battle-status battle-status-${battleState.type}`} key={battleState.type} title={battleState.name}>{battleState.icon} {battleState.name}{battleState.stacks > 1 ? ` x${battleState.stacks}` : ''}{battleState.turns ? ` · ${battleState.turns}` : ''}</span>)}</div>}
      <div className="battle-character-image">{isActive && <div className="battle-turn-badge">⚔️ {isMine ? '¡TU TURNO!' : '¡TURNO DEL OPONENTE!'}</div>}{state?.defending && <div className="battle-defense-badge">🛡️ DEFENDIENDO</div>}{image ? <img src={image} alt={displayCharacter?.name || 'Personaje'} /> : <div className="battle-character-placeholder">{displayCharacter?.name?.[0] || '?'}</div>}</div>
    </article>
  )
}
...
export default function OnlineBattlePage() {
  ...
  useEffect(() => {
    ...
    refreshRoom(room.id)
    const refreshParticipantsAndCharacters = async () => { if (disposed || fullRefreshInFlight) return; fullRefreshInFlight = true; try { await refreshRoom(room.id, { includeParticipants: true }) } finally { fullRefreshInFlight = false } }
    const channel = supabase.channel(`battle-room-${room.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'battle_rooms', filter: `id=eq.${room.id}` }, payload => {
        if (disposed) return
        if (payload.eventType === 'DELETE') { setRoom(null); setParticipants([]); setBattleCharacters([]); battleCharacterIdsRef.current = ''; return }
        if (payload.new) { setRoom(payload.new); if (payload.new.status === 'active' || payload.new.status === 'finished') loadBattleCharactersFromState(payload.new.battle_state); if (payload.new.status === 'ready') { setBattleCharacters([]); battleCharacterIdsRef.current = '' }; if (payload.new.status === 'active' && battleCharactersRef.current.length === 0) refreshParticipantsAndCharacters() }
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'battle_participants', filter: `room_id=eq.${room.id}` }, () => { if (!disposed) refreshParticipantsAndCharacters() })
      .subscribe((status, subscriptionError) => { realtimeHealthy = status === 'SUBSCRIBED'; if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') console.warn('Supabase Realtime no disponible para la sala:', subscriptionError) })
    const fallbackPoll = window.setInterval(() => { if (!disposed && !realtimeHealthy) refreshRoom(room.id, { includeParticipants: false }) }, 10000)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    async function awardOnlineXp() { const { data, error: rewardError } = await supabase.rpc('award_online_battle_xp', { p_room_id: room.id }); if (cancelled) return; if (rewardError) { setError(rewardError.message); return }; const reward = Array.isArray(data) ? data[0] : data; setXpReward(reward || null) }
    awardOnlineXp()
  ...
  async function createRoom() {
    ...
    if (participantError) { setError(participantError.message); await supabase.from('battle_rooms').delete().eq('id', data.id) } else { setRoom(data); setParticipants([{ user_id: user.id, role: 'host', character_id: null, rematch_status: 'pending' }]) }
    setLoading(false)
  }
  ...
  async function joinRoom() {
    ...
    else { const { data: updated, error: updateError } = await supabase.from('battle_rooms').update({ status: 'ready' }).eq('id', data.id).eq('status', 'waiting').select().single(); if (updateError) setError(updateError.message); else { setRoom(updated || data); await refreshRoom(data.id) } }
    setLoading(false)
  }
  ...
  async function startOnlineBattle() {
    ...
    const hostHp = getMaxHp(hostCharacter); const guestHp = getMaxHp(guestCharacter)
    const firstUserId = getSpeed(hostCharacter) >= getSpeed(guestCharacter) ? host.user_id : guest.user_id
    const battleState = { version: 1, round: 1, turn_user_id: firstUserId, players: { [host.user_id]: { user_id: host.user_id, role: 'host', character_id: host.character_id, hp: hostHp, max_hp: hostHp, energy: 0, defending: false, states: [] }, [guest.user_id]: { user_id: guest.user_id, role: 'guest', character_id: guest.character_id, hp: guestHp, max_hp: guestHp, energy: 0, defending: false, states: [] } }, winner_user_id: null, status: 'active', log: [] }
  ...
  async function performOnlineAction(action = selectedAction) {
    ...
    setLoading(true); setError('')
    let rpcAction = action; let abilityIndex = null
  ...
  async function resetForRematch() {
    ...
    rematchResetInFlight.current = true; setLoading(true); setError('')
    ...
    if (participantsError) { setError(participantsError.message); setLoading(false); rematchResetInFlight.current = false; return }
    ...
    if (roomError) { setError(roomError.message); setLoading(false); rematchResetInFlight.current = false; return }
    setRoom(data); setParticipants(previous => previous.map(participant => ({ ...participant, character_id: null, rematch_status: 'pending' }))); setBattleCharacters([]); battleCharacterIdsRef.current = ''; setSelectedCharacterId(null); setSelectedAction('basic'); setXpReward(null); rewardRoomRef.current = null; setLoading(false); rematchResetInFlight.current = false
  }
  ...
  const myDisplayCharacter = getBattleCharacterForDisplay(myBattleCharacter, myBattlePlayer)
  const isMyTurn = battleState?.turn_user_id === user?.id && battleState?.status !== 'finished' && room?.status !== 'finished'
  const myAbilities = getBattleAbilities(myBattleCharacter, myBattlePlayer)
  ...
  const turnBaseCharacter = turnParticipant ? battleCharacters.find(c => c.id === battleState?.players?.[turnParticipant.user_id]?.character_id) : null
  const turnCharacter = turnParticipant ? getBattleCharacterForDisplay(turnBaseCharacter, battleState?.players?.[turnParticipant.user_id]) : null

  useEffect(() => { if (!room?.id || room.status !== 'finished' || !host || !guest) return; if (host.rematch_status === 'accepted' && guest.rematch_status === 'accepted' && room.host_user_id === user?.id) resetForRematch() }, [room?.id, room?.status, host?.rematch_status, guest?.rematch_status, user?.id])
    ...
    return (
      <section className="battle-page online-battle-page">
        <div className="battle-heading">
          ...
          {!isFinished && <button className="button secondary" type="button" onClick={() => setRoom(null)}>Salir</button>}
        </div>
        <div className={`battle-arena online-arena ${isFinished ? 'battle-arena-phase-result' : 'battle-arena-phase-fighting'}`}>
          <div className="battle-round"><span>ROUND {battleState?.round || 1}</span><strong>{isFinished ? (didWin ? '🏆 ¡COMBATE TERMINADO!' : '💀 ¡COMBATE TERMINADO!') : `Turno de ${turnCharacter?.name || (isMyTurn ? 'vos' : 'tu oponente')}`}</strong></div>
          <div className="battle-fighters">
            {host && <OnlineBattleCharacterCard participant={host} state={battleState?.players?.[host.user_id] || {}} character={battleCharacters.find(c => c.id === battleState?.players?.[host.user_id]?.character_id)} side="left" isMine={host.user_id === user?.id} isActive={!isFinished && host.user_id === battleState?.turn_user_id} />}
            <div className="battle-vs">VS</div>
            {guest && <OnlineBattleCharacterCard participant={guest} state={battleState?.players?.[guest.user_id] || {}} character={battleCharacters.find(c => c.id === battleState?.players?.[guest.user_id]?.character_id)} side="right" isMine={guest.user_id === user?.id} isActive={!isFinished && guest.user_id === battleState?.turn_user_id} />}
          </div>
          {!isFinished && <div className={`battle-action-panel ${!isMyTurn ? 'is-opponent-turn' : ''}`} key={battleState?.turn_user_id}>
          ...
          <div className="battle-log online-log"><div className="battle-log-header"><p className="eyebrow">Registro del combate</p></div>{(battleState?.log || []).slice().reverse().map(entry => <div className={`battle-log-entry battle-log-${entry.type || 'attack'}`} data-log-id={entry.id || ''} data-log-type={entry.type || 'attack'} data-actor-user-id={entry.user_id || ''} key={entry.id}>{entry.message || entry.text}</div>)}</div>
        </div>
        {isFinished && <BattleResultScreen result={didWin ? 'victory' : 'defeat'} character={myDisplayCharacter || myBattleCharacter} xpEarned={xpReward?.experience_gained || 0} currentLevel={xpReward?.new_level || 1} previousLevel={xpReward?.previous_level || xpReward?.new_level || 1} leveledUp={Boolean(xpReward?.leveled_up)} unlockedCharacter={unlockedCharacter} stats={stats} isOnline rematchStatus={myRematchStatus} opponentRematchStatus={opponentRematchStatus} rematchLoading={loading} onRequestRematch={requestRematch} onDeclineRematch={declineRematch} onBack={() => { setRoom(null); setXpReward(null); rewardRoomRef.current = null }} />}
      </section>
    )
  }
  ...
  return (
    <main className="online-battle">
      ...
      {room && <section className="battle-room panel"><div className="room-code"><span>CÓDIGO DE SALA</span><strong>{room.code}</strong></div><div className="room-status"><span>ESTADO</span><strong>{room.status === 'ready' ? 'SELECCIÓN DE PERSONAJE' : 'ESPERANDO AL OPONENTE...'}</strong></div><div className="players"><article><span>HOST</span><strong>{hostName}</strong>{host?.character_id && <small>✓ Personaje elegido</small>}</article><div>VS</div><article><span>OPONENTE</span><strong>{guestName}</strong>{guest?.character_id && <small>✓ Personaje elegido</small>}</article></div>{room.status === 'ready' && <div className="character-select"><div className="character-select__heading"><div><p className="eyebrow">TU COLECCIÓN</p><h2>Elegí tu personaje</h2></div><span>{selectedCharacter ? `Elegido: ${selectedCharacter.name}` : 'Ninguno elegido'}</span></div><div className="character-grid">{characters.map(character => { const image = getImage(character); const selected = character.id === selectedCharacterId; return <button key={character.id} className={`character-card ${selected ? 'is-selected' : ''}`} onClick={() => selectCharacter(character.id)} disabled={loading} type="button">{image ? <img src={image} alt={character.name} /> : <div className="character-card__fallback">{character.name.charAt(0)}</div>}<strong>{character.name}</strong>{selected && <span>✓ ELEGIDO</span>}</button> })}</div><div className="selection-status">{bothSelected ? '✓ Ambos jugadores eligieron personaje' : 'Esperando la elección del otro jugador...'}</div></div>}{bothSelected && <button className="start-button" onClick={startOnlineBattle} disabled={loading || user?.id !== room.host_user_id} type="button">{user?.id === room.host_user_id ? 'COMENZAR BATALLA' : 'ESPERANDO AL HOST...'}</button>}</section>}
    </main>
  )
}

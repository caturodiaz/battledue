...
function OnlineBattleCharacterCard({ character, participant, state, isActive, isMine, side }) {
  ...
  return (
    <article className={`battle-character battle-character-${side} ${isActive ? 'is-active' : ''} ${state?.defending ? 'is-defending' : ''} ${hp <= 0 ? 'is-defeated' : ''}`} data-user-id={participant.user_id} data-character-id={state?.character_id || ''} data-ultimate-image={character?.profile?.ultimateImage || ''} data-ultimate-name={character?.profile?.ultimateName || 'Técnica definitiva'}>
      <div className="battle-character-heading"><div><p className="battle-side-label">{isMine ? 'VOS' : 'OPONENTE'}</p><h2>{character?.name || 'Personaje'}</h2></div><strong className="battle-hp-value">{Math.ceil(hp)}</strong></div>
      ...
      <div className="battle-character-image">{isActive && <div className="battle-turn-badge">⚔️ {isMine ? '¡TU TURNO!' : '¡TURNO DEL OPONENTE!'}</div>}{state?.defending && <div className="battle-defense-badge">🛡️ DEFENDIENDO</div>}{image ? <img src={image} alt={character?.name || 'Personaje'} /> : <div className="battle-character-placeholder">{character?.name?.[0] || '?'}</div>}</div>
    </article>
  )
}
